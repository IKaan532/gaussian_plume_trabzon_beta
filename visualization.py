...
def _draw_road_lines(fig: "go.Figure", roads: list[dict]) -> None:
    ...
    lats_by_class: dict[str, list] = {}
    lons_by_class: dict[str, list] = {}

    for road in roads:
        hw = road["tags"].get("highway", "residential")
        for key in road_colors_map:
        ...
        node_lats = [n[0] for n in road["nodes"]] + [None]
        node_lons = [n[1] for n in road["nodes"]] + [None]
        lats_by_class.setdefault(hw, []).extend(node_lats)
        lons_by_class.setdefault(hw, []).extend(node_lons)

    for cls in lats_by_class:
        fig.add_trace(
            go.Scattermapbox(
                lat       = lats_by_class[cls],
                lon       = lons_by_class[cls],
                mode      = "lines",
